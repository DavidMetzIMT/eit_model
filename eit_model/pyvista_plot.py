...
class PyVistaPlotWidget(MainWindow):
    def __init__(self, eit_mdl: EITModel, parent=None, show=True):
        ...
        self.ui.action_mesh_dynamic_slicing_y.triggered.connect(
            lambda: self._mesh_dynamic_slicing(1)
        )
        self.ui.action_mesh_dynamic_slicing_z.triggered.connect(
            lambda: self._mesh_dynamic_slicing(2)
        )
        ...
        self.ui.action_onoff_parallel_projection.triggered.connect(
        ...
        self.ui.action_show_elements_data_monitoring.triggered[bool].connect(
            self.ui.tabW_elem_data.setVisible
        )
        self.ui.action_show_elements_data_monitoring.setChecked(True)

        self.ui.slider_x.valueChanged[int].connect(
            lambda val: self._update_slice_origin(val, 0)
        )
        self.ui.slider_x.installEventFilter(self)
        self.ui.slider_y.valueChanged[int].connect(
            lambda val: self._update_slice_origin(val, 1)
        )
        self.ui.slider_y.installEventFilter(self)
        self.ui.slider_z.valueChanged[int].connect(
            lambda val: self._update_slice_origin(val, 2)
        )
    ...
    def eventFilter(self, source: QtCore.QObject, event: QtCore.QEvent) -> bool:
        ...
        if (
            isinstance(source, QtWidgets.QSlider)
    ...
    def new_data(self, data=None):
        if not isinstance(self.eit_image, EITImage):
            self.eit_image = build_EITImage(
                data=None, label="default", model=self.eit_mdl
            )
        img = self.eit_image
        img.data = (
            np.random.random_sample(self.chamber.cell_data["Conductivity"].shape) * 3
        )
    ...
    def _plot_eit(self):
        ...
        self.plotter.add_scalar_bar(
            "Conductity", interactive=False, vertical=False, color=[0, 0, 0]
        )
        # self.ortho_slice()
        self.plotter.show_bounds(
            grid="front", location="outer", all_edges=True, color=[0, 0, 0]
        )
    ...
    def _mesh_dynamic_slicing(self, idx: int):
        ...
        self.plotter.add_mesh_slice(
            self.chamber, assign_to_axis=idx, name=name, cmap=self.cmap
        )
        self.plotter.add_text(
            text="Mesh dynamic slicing", font_size=10, name="text_main"
        )
        self.actors[name] = None
